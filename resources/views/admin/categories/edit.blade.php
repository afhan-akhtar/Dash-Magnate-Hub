...
@section('page_subtitle', 'Update category details and settings.')

@push('styles')
<style>
    .admin-category-form-shell .profile-form-card {
        border: 0;
        box-shadow: 0 18px 45px rgba(40, 60, 80, 0.08);
    }

    .admin-category-form-shell .profile-form-card .card-header {
    ...
    .admin-category-form-shell .profile-form-card .card-header .card-title {
        margin-bottom: 0;
    }

    .admin-category-form-shell .profile-form-card .card-body {
        padding: 1.5rem;
    }

    .admin-category-form-shell .profile-form-note {
        padding: 1rem 1.1rem;
        ...
        <div class="page-header">
            <div class="page-header-left d-flex align-items-center">
                ...
                <ul class="breadcrumb">
                    ...
                    <li class="breadcrumb-item">{{ \Illuminate\Support\Str::limit($category->name, 40) }}</li>
                </ul>
            </div>
        </div>

        <div class="main-content admin-category-form-shell">
                    ...
                    <form method="POST" action="{{ route('admin.categories.update', $category->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="card stretch stretch-full profile-form-card mb-4">
                            <div class="card-header">
                                ...
                                <p class="text-muted fs-12 mb-0">Update the display name and URL path for this category.</p>
                            </div>
                            <div class="card-body">
                                <div class="row g-4">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control @error('name') is-invalid @enderror"
                                               ...
                                               placeholder="e.g. Accommodation & Tourism" required>
                                        @error('name')
                        ...
                        <div class="card stretch stretch-full profile-form-card mb-4">
                            <div class="card-header">
                                ...
                                <p class="text-muted fs-12 mb-0">Control whether the category is enabled and shown on the site.</p>
                            </div>
                            <div class="card-body">
                                <div class="row g-4">
                                    <div class="col-sm-6">
                                        <div class="d-flex align-items-center justify-content-between p-3 rounded border bg-light-subtle">
                                            <div>
                                                ...
                                                <small class="text-muted">Enable or disable this category</small>
                                            </div>
                                    ...
                                    <div class="col-sm-6">
                                        <div class="d-flex align-items-center justify-content-between p-3 rounded border bg-light-subtle">
                                            <div>
                                                ...
                                                <small class="text-muted">Show this category publicly</small>
                                            </div>
                                            <div class="form-check form-switch mb-0">
                                                <input class="form-check-input" type="checkbox" role="switch"
                        ...
                        <div class="card stretch stretch-full profile-form-card mb-4">
                            <div class="card-header">
                                <h5 class="card-title mb-0">
                                    <i class="feather-image me-2 text-primary"></i>Category image
                                </h5>
                                <p class="text-muted fs-12 mb-0">Replace the thumbnail used on cards and listings.</p>
                            </div>
                            <div class="card-body">
                                @php
                                    $categoryThumb = $category->thumbnail;
                                    $categoryThumbPublicUrl = null;
                                    if ($categoryThumb) {
                                        $rawPath = $categoryThumb->path ? str_replace('\\', '/', $categoryThumb->path) : null;
                                        if ($rawPath) {
                                            // Rebuild from path + current request base so XAMPP/subfolder & host match the open tab (stored `url` column is often stale/wrong).
                                            $base = rtrim(request()->getBaseUrl(), '/');
                                            $categoryThumbPublicUrl = $base !== ''
                                                ? $base . '/storage/' . ltrim($rawPath, '/')
                                                : asset('storage/' . ltrim($rawPath, '/'));
                                        }
                                        if ($categoryThumbPublicUrl === null && ! empty($categoryThumb->url)) {
                                            $categoryThumbPublicUrl = $categoryThumb->url;
                                        }
                                    }
                                @endphp
                                <div class="row g-4 align-items-start">

                                    {{-- Current thumbnail: URL from filesystem path so preview actually loads --}}
                                    @if ($categoryThumb && $categoryThumbPublicUrl)
                                    <div class="col-lg-5">
                                        <div class="border rounded-3 shadow-sm overflow-hidden h-100">
                                            ...
                                            <div class="admin-doc-thumb-preview bg-white p-4 text-center position-relative">
                                                <img src="{{ $categoryThumbPublicUrl }}"
                                                     alt="{{ $category->name }}"
                                    ...
                                    @endif
                                        <label class="form-label">
                                            {{ $categoryThumb ? 'Replace image' : 'Upload image' }}
                                        </label>
                        ...
                        <div class="d-flex justify-content-between align-items-center gap-3 mb-5">
                            <a href="{{ route('admin.categories.index') }}" class="btn btn-light-brand px-4">
