...
<script>
    ...
    // function Get_Category() {$.ajax({method: "GET",url: '/Location/Category'}).done(function(response) {
    // $("#category_id").html(' ').append(`<option value="0">Select City</option>`);
    // var category = response.Category;category.forEach(function(c) {$("#category_id").append(`<option value="${c.category_id}">${c.name}</option>`);});});}

    // Create Jquery
    function Create(event) {
        event.preventDefault();

        ALL = ['name', 'category_id']; var fd = new FormData(); fd.append('_token', $("input[name=_token]").val());
        for (let i = 0; i < ALL.length; i++) {fd.append(ALL[i], $("#" + ALL[i]).val());}
        var card = $("#card")[0].files; for (var i = 0; i < card.length; i++) {fd.append("card[]", card[i], card[i]['name']);}
        $.ajax({method: "POST",url: '/Location/Insert',processData: false,contentType: false,data: fd,
        beforeSend: function() {$('#Saved_Button').attr('disabled', 'disabled').html(' ').append(`<div class="Button_Loader"></div>`);}})
        .done(function(response) {
    ...
    $.ajax({method: "POST",url: '/Location/Delete',processData: false,contentType: false,data: fd}).done(function(response) {if (response.error == true) {location.assign("/dashboard/login");}if (response.error == false) {$('.no').click(); Get(); Sweet_Alert('Congratulation',response.message,'success');}});}
